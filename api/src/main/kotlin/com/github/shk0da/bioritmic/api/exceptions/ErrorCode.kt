...
enum class ErrorCode(val code: String, val message: String, val httpCode: HttpStatus) {
    ...
    REQUIRED_PARAMETER(
        "API-400.2",
        "Parameter [\${${Constants.PARAMETER_NAME}}] value is required.",
        HttpStatus.BAD_REQUEST
    ),
    INVALID_PARAMETER(
        "API-400.3",
        "Parameter [\${${Constants.PARAMETER_NAME}}] value is invalid.",
        HttpStatus.BAD_REQUEST
    ),
    INVALID_PARAMETER_WITH_VALUE(
        "API-400.4",
        "Parameter [\${${Constants.PARAMETER_NAME}}] with requested value " +
            "[\${${Constants.PARAMETER_VALUE}}] is invalid.",
        HttpStatus.BAD_REQUEST
    ),
    INVALID_PARAMETER_SIZE(
        "API-400.5",
        "The parameter [\${${Constants.PARAMETER_NAME}}] cannot be longer than " +
            "[\${${Constants.PARAMETER_VALUE_LENGTH}}] characters.",
        HttpStatus.BAD_REQUEST
    ),
    INVALID_PARAMETER_RANGE(
        ...
        "Parameter [\${${Constants.PARAMETER_NAME}}] value is invalid. " +
            ...
            "[\${${Constants.PARAMETER_VALUE_START}}-\${${Constants.PARAMETER_VALUE_END}}].",
        HttpStatus.BAD_REQUEST
    ),
    ...
    USER_WITH_EMAIL_NOT_FOUND(
        "API-404",
        "User with email: [\${${Constants.PARAMETER_VALUE}}] not found.",
